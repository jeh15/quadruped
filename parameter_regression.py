...
def main(argv=None):
    ...
    # Potential Parameters to Regress:
    ...
    initial_params = {
        ...
        'dof_frictionloss': frictionloss_params,
        'kp': kp_params,
    }
    ...
    def inner_loop(carry, xs):
        ...
        gradient = {
            ...
            'dof_frictionloss': grad.dof_frictionloss,
    ...
    dof_damping_history = np.concatenate(param_history['dof_damping'])
    dof_frictionloss_history = np.concatenate(param_history['dof_frictionloss'])
    kp_history = np.concatenate(param_history['kp'])

    # Run initial comparison of random trial:
    gain = sys.actuator_gainprm.at[:, 0].set(initial_params['kp'])
    bias = sys.actuator_gainprm.at[:, 0].set(-initial_params['kp'])
    sys = sys.replace(
        dof_damping=initial_params['dof_damping'],
        dof_frictionloss=initial_params['dof_frictionloss'],
        actuator_gainprm=gain,
        actuator_biasprm=bias,
    )
    ...
    for i in range(setpoints.shape[0]):
    ...
    gain = sys.actuator_gainprm.at[:, 0].set(params['kp'])
    bias = sys.actuator_gainprm.at[:, 0].set(-params['kp'])
    sys = sys.replace(
        dof_damping=params['dof_damping'],
        dof_frictionloss=params['dof_frictionloss'],
        actuator_gainprm=gain,
    ...
    fig, ax = plt.subplots(4, 1, constrained_layout=True, figsize=(10, 5))
    ...
    ax[1].plot(dof_frictionloss_history[:, 0], color='orange', label='Abduction', linewidth=1.0)
    ax[1].plot(dof_frictionloss_history[:, 1], color='cornflowerblue', label='Hip', linewidth=1.0)
    ax[1].plot(dof_frictionloss_history[:, 2], color='lightcoral', label='Knee', linewidth=1.0)
    ax[1].legend()
    ax[1].set_title('Friction Regression')
    ax[1].set_xlabel('Iterations')
    ax[1].set_ylabel('Friction Loss Parameter')

    ax[2].plot(kp_history [:, 0], color='orange', label='Abduction', linewidth=1.0)
    ax[2].plot(kp_history[:, 1], color='cornflowerblue', label='Hip', linewidth=1.0)
    ax[2].plot(kp_history[:, 2], color='lightcoral', label='Knee', linewidth=1.0)
    ax[2].legend()
    ax[2].set_title('Motor Gain Regression')
    ax[2].set_xlabel('Iterations')
    ax[2].set_ylabel('Motor Gain Parameter')

    ax[-1].plot(loss_history, linewidth=1.0)
    ax[-1].set_title('Loss')
    ax[-1].set_xlabel('Iterations')
