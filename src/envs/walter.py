...
class WalterEnv(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ctrl_lb = jnp.array([-jnp.pi / 4, -jnp.pi / 4, -3.0, -3.0] * 4)
        self.ctrl_ub = jnp.array([jnp.pi / 4, jnp.pi / 4, 3.0, 3.0] * 4)
        ...
        thigh_body_ids = [
            mujoco.mj_name2id(sys.mj_model, mujoco.mjtObj.mjOBJ_BODY, thigh_body)
            for thigh_body in thigh_bodies
        ]
        self.thigh_body_ids = jnp.array(thigh_body_ids)
    ...
    def reset(self, rng: PRNGKey) -> State:
        ...
        state_info = {
            ...
            'wheel_air_time': jnp.zeros(self.wheel_site_ids.shape),
    ...
    def numpy_observation(
        ...
        inverse_base_rotation = quat_inv(base_w)
        local_yaw_rate = rotate(base_dw, inverse_base_rotation)[2]
        ...
        new_observation = np.concatenate([
            np.array([local_yaw_rate ]),
            projected_gravity,
            command,
            joint_q - self.default_pose,
            previous_action,
        ])
