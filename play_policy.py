...
def main(argv=None):
    ...
    for velocity in velocities:
        ...
        for i in range(num_steps):
            ...
            radius_vector = state.pipeline_state.xpos[xpos_hip_idx] - state.pipeline_state.site_xpos[env.feet_site_idx]
        ...
        print(f'Average Body Height: {q_body_height:.3f}')

        # Calculate COT:
        power = np.sum(np.trapz(torques ** 2, dx=env.step_dt, axis=0), axis=-1)
        time = env.step_dt * num_steps
        cot = (power * time) / (jnp.sum(env.sys.body_mass) * jnp.linalg.norm(env.sys.gravity) * state.metrics['total_distance'])
